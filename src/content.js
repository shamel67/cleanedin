...
(() => {
    ...
    function processElement(node, settingId, setting) {
        ...
        if (cleanedIn === 0 || !setting.status) return;

        const classToApply = setting.class || (cleanedIn === 1 ? ELEMENT_CLASS : REMOVE_CLASS);
        node.classList.add(classToApply);

        if (setting.class === "CleanedInCustom" && setting.color) {
    ...
    /**
     * Initialize CleanedIn on page load
     */
    function initialize() {
